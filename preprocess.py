...
parser.add_argument("-sup_valid_src", required=True,
                    help="Path to the validation source data")
parser.add_argument("-sup_valid_tgt", required=True,
                     help="Path to the validation target data")

parser.add_argument("-bandit_valid_src", required=True,
...
def makeData(which, srcFile, tgtFile, srcDicts, tgtDicts, no_filter):
    src, tgt = [], []
    sizes = []
    count, ignored = 0, 0
    ...
    while True:
        ...
        if not no_filter:
            # truncate sentences with length >= seq_length e.g. training data
            ...
            # include all sentences e.g. in the case of valid and test data set
            src += [srcDicts.convertToIdx(srcWords,
                                            lib.Constants.UNK_WORD)]
            tgt += [tgtDicts.convertToIdx(tgtWords,
                                            lib.Constants.UNK_WORD,
...
def makeDataGeneral(which, src_path, tgt_path, dicts, no_filter=False):
    print("Preparing " + which + "...")
    res = {}
    res["src"], res["tgt"], res["pos"] = makeData(which, src_path, tgt_path,
        dicts["src"], dicts["tgt"], no_filter)
    return res
...
def main():
    ...
    save_data["sup_valid"] = makeDataGeneral("sup_valid", opt.sup_valid_src, opt.sup_valid_tgt,
